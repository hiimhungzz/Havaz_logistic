 ...
 *
 */
const OrderModal = ({profile, modal, handleShowModal, uuid, setUuid, isAddToCTable, onAddToCTable, setParams, definitions}) => {
  ...
  let { undelivered_reasons } = definitions.toJS()
  console.log('undelivered_reasons', undelivered_reasons)
  let undeliveredReasons = _.map(undelivered_reasons, (value, k) => {
      return value
  })
  const [lyDoHuyDon, set_lyDoHuyDon] = useState(undeliveredReasons)
  ...
  const CancelOrder = useCallback(async () => {
    ...
    if (result.hasErrors) {
      ...
      Ui.showSuccess({ message: 'Hủy hàng thành công'});
  ...
  const [_lyDoXoaDonHang, Set_lyDoXoaDonHang] = useState(3)
  ...
  const footerConten = function () {
    if (uuid === '') {
      ...
      return (
        <Row justify="end" gutter={[8, 8]}>
          <Space size={10} align="center">
            <Button onClick={showModal}>Phát không thành công</Button>
            <Button type="primary" onClick={UpdateOrder}>Phát hàng</Button>
          </Space>
          <Modal
