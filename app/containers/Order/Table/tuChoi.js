import { DeleteTwoTone, LeftSquareOutlined, PrinterOutlined } from "@ant-design/icons";
...
const tableOrder = memo(
  ...
  ({
    ...
    let columns = [
      {
        ...
        render: (text, record) => (
          <div className="d-flex" style={{ flexDirection: "column" }}>
            <p style={{ marginBottom: "0em" }}> {record["exporting_order"]['exporting_id']}</p>
          </div>
        )
      },
      {
        ...
        render: (text, record) => (
          <>
            ...
            <Button
              ...
              type="link"
              onClick={() => {
                Set_uuidXoaDonHang(record);
                Set_lyDoXoaDonHang(record.code);
    ...
    const CancelOrder = useCallback(async (_id, exportingId) => {
      ...
      const result = await ServiceBase.requestJson({
        method: "POST",
        url: `/v1/exportings/${exportingId}/items/${_id}/reimport-order`,
        data: {
      ...
      if (result.hasErrors) {
        ...
        Ui.showSuccess({ message: "Nhận đơn hàng thành công." });
        Set_visibleOrder(false);
        onLoad()
      }
    }, [_lyDoXoaDonHang]);
    const huyDonHang = () => {
      CancelOrder(_uuidXoaDonHang?.id, _uuidXoaDonHang?.exporting_order?.exporting_id);
    };
        ...
        <Row justify="end" gutter={[8, 8]}>
          <Col style={{ marginTop: "10px" }}>
            <Form form={form}>
              ...
              >
                <span>Bạn muốn nhận đơn hàng có mã  {_lyDoXoaDonHang} về kho nhận ?</span>
