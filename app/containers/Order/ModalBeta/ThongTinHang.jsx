...
const AddOn = ({
    ...
    const pushAddOn = useCallback(async () => {
        ...
        SetAddOn([...addOn, {id: key ,select: `${rowKey}_select_${key}`, input_num_of_package: `${rowKey}_input_${key}`, input_quantity: `${rowKey}_input_${key}_quantity}`,  input_fee: `${rowKey}_input_${key}_fee}`}])
    },[addOn])
    ...
    useEffect(() => {
        ...
        let formData = {};
        _.forEach(DataBin, (i) => {
            tempAddOn.push({id: index ,select: `${rowKey}_select${index}`, input_num_of_package: `${rowKey}_input${index}`, input_quantity: `${rowKey}_input_${index}_quantity}`,  input_fee: `${rowKey}_input_${index}_fee}`})
                    ...
                    return (
                        <Row className={className}>
                            ...
                            <Col span={2} className="type-custome">
                                <span>{_service_type_product[dataAddOn[i['id']]?.type_of] ? `(${_service_type_product[dataAddOn[i['id']]?.type_of].unit?.text})` : 'N/A'}</span>
                            </Col>
                            <Col span={4} >
