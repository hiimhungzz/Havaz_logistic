      ...
      <Row justify="start" className={className}>
        ...
        <Col span={23} className="col-23-fix">
          <Row gutter={[24, 32]}>
            ...
            <Col span={7} >Tiền cước phải thu được <span className={'tien-cuoc'} style={{color: '#1890ff'}}>{convert(tongCuocNguoiNhanTra(objOrder)) || 0}</span></Col>
            <Col span={7}  >Tiền COD phải thu được <span className={'tien-cuoc'} style={{color: '#1890ff'}}>{convert(objOrder["order_cod"])  || 0}</span></Col>
            <Col span={7} ><span style={{color: 'rgba(255, 24, 24, 0.93)'}}>Tổng phải thu <span className={'tien-cuoc'}>{convert((tongCuocNguoiNhanTra(objOrder) + objOrder["order_cod"]))  || 0}</span></span></Col>
            <Col span={24} >
